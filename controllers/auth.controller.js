...
// @access  Public
// @desc    Register new fleet manager
...
exports.register = async (req, res) => {
  try {
    ...
    const FleetManager = require('../models/FleetManager');
    const existing = await FleetManager.findOne({ email });
    if (existing) {
      return res.status(409).json({ success: false, message: 'Email already registered.' });
    }
  const manager = new FleetManager({ name, email, password });
    await manager.save();
...
// @desc    Get logged-in manager profile  Test
